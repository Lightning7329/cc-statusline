...
let ``session_name がないとき None になる`` () =
    let ctx = parseInput (fullJson.Replace(""""session_name": "my-session",""", ""))

    ctx.SessionName |> should equal None

[<Fact>]
let ``vim がないとき None になる`` () =
    let ctx = parseInput (fullJson.Replace(""""vim": { "mode": "NORMAL" },""", ""))
    ctx.Vim |> should equal None

[<Fact>]
let ``agent がないとき None になる`` () =
    let ctx =
        parseInput (fullJson.Replace(""""agent": { "name": "security-reviewer" },""", ""))
