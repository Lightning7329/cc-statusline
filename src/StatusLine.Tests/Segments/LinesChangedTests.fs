...
let ``追加と削除を プラス スラッシュ マイナス の3片で返す`` () =
    let spans = format (makeCost 5 3)
    spans |> List.map _.Text |> should equal [ "+5"; "/"; "-3" ]

[<Fact>]
let ``追加片は緑色になる`` () =
    let span = format (makeCost 5 3) |> List.head
...
[<Fact>]
let ``区切りのスラッシュは無色になる`` () =
    let span = format (makeCost 5 3) |> List.item 1
    span.Text |> should equal "/"
    span.Color |> should equal None

[<Fact>]
let ``削除片は赤色になる`` () =
    let span = format (makeCost 5 3) |> List.last
